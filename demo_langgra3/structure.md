.
├── index.html          # 앱의 메인 엔트리 포인트: 전체 레이아웃 구조(사이드바, 캔버스, 인스펙터) 정의, Inspector에 Config/Result 탭 구조 포함, 모달/컨텍스트 메뉴/툴팁 등 오버레이 요소의 스켈레톤 포함 및 모든 CSS/JS 리소스 로드 담당
├── backend.py          # FastAPI 기반 백엔드 실행 엔진: LangGraph DAG로 셀 당 코드 생성(generate) → 의도 검증(validate_intent) → 실행 검증(execute) → 래핑(wrap) → Docstring → 갈무리(cleanup) → 결과 반환(finalize) 7단계 파이프라인 실행. Anthropic/Ollama 모델 분기, MemorySaver 체크포인트, SSE 스트리밍 응답 담당. 엔드포인트: POST /cell/run, /cell/stop/{id}, /cell/resume, GET /cell/state/{id}
├── css/                # 스타일시트 디렉토리
...
└── js/                 # 자바스크립트 디렉토리
    ├── canvas.js       # 캔버스 코어 및 노드 조작 로직: 노드 생성(Create/Clone) 및 삭제, 드래그 앤 드롭 팔레트 인터페이스, 베지에 곡선 기반 엣지(연결선) 렌더링, 줌 제어 및 좌표 변환 로직 담당
